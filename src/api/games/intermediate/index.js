...
interGamesRouter.post("/intermediate/second", async (req, res, next) => {
    try {
        const newGame = new InterGamesModel(req.body)
        const { _id } = await newGame.save()
        res.status(201).send({ _id })
...
interGamesRouter.get("/intermediate/second/:id", async (req, res, next) => {
    try {
        const game = await InterGamesModel.findById(req.params.id);
        if (game) {
            res.send(game)
...
interGamesRouter.put("/intermediate/second/:id", async (req, res, next) => {
    try {
        const updatedGame = await InterGamesModel.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true })
        if (updatedGame) {
            res.send(updatedGame)
...
interGamesRouter.delete("/intermediate/second/:id", async (req, res, next) => {
    try {
        const deletedGame = await InterGamesModel.findByIdAndDelete(req.params.id)
        if (deletedGame) {
            res.status(204).send()
